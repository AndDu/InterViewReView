...
public class BaseImageDecoder implements ImageDecoder {
    ...
    public Bitmap decode(ImageDecodingInfo decodingInfo) throws IOException {
        ...
        InputStream imageStream = getImageStream(decodingInfo); //通过各种方法获取图片流(方法待深入)
        if (imageStream == null) {
            L.e(ERROR_NO_IMAGE_STREAM, decodingInfo.getImageKey());
            return null;
        }
        try {
            imageInfo = defineImageSizeAndRotation(imageStream, decodingInfo); //定义图片的尺寸和转向
            imageStream = resetStream(imageStream, decodingInfo);  //重新调用了getstream方法，同时还调用了inputsteam 的reset方法。点解？
            Options decodingOptions = prepareDecodingOptions(imageInfo.imageSize, decodingInfo);    //获取到图片的压缩倍数
            decodedBitmap = BitmapFactory.decodeStream(imageStream, null, decodingOptions);//获取图片的bitmap
        } finally {
        ...
        if (decodedBitmap == null) {
            ...
            decodedBitmap = considerExactScaleAndOrientatiton(decodedBitmap, decodingInfo, imageInfo.exif.rotation,
                    imageInfo.exif.flipHorizontal);//这个方法是精确的缩放和图片的方向
        }
    ...
    protected ImageFileInfo defineImageSizeAndRotation(InputStream imageStream, ImageDecodingInfo decodingInfo)
        ...
        if (decodingInfo.shouldConsiderExifParams() && canDefineExifParams(imageUri, options.outMimeType)) {//这个图片解码待研究
    ...
    protected Options prepareDecodingOptions(ImageSize imageSize, ImageDecodingInfo decodingInfo) {
        ...
        if (scaleType == ImageScaleType.NONE) {
            ...
            scale = ImageSizeUtils.computeMinImageSampleSize(imageSize);  //根据目标大小和图片的实际大小相除向上取整来获取缩放参数，
        } else {
            ImageSize targetSize = decodingInfo.getTargetSize();
            boolean powerOf2 = scaleType == ImageScaleType.IN_SAMPLE_POWER_OF_2;
            scale = ImageSizeUtils.computeImageSampleSize(imageSize, targetSize, decodingInfo.getViewScaleType(), powerOf2);//根据memoryCacheExtraOptions设置的图片大小来获取缩放图片的倍数。
        }
        ...
        decodingOptions.inSampleSize = scale; //bitmap 的缩放压缩方法。
    ...
    protected Bitmap considerExactScaleAndOrientatiton(Bitmap subsampledBitmap, ImageDecodingInfo decodingInfo,
        ...
        if (scaleType == ImageScaleType.EXACTLY || scaleType == ImageScaleType.EXACTLY_STRETCHED) {
            ...
            if (Float.compare(scale, 1f) != 0) {
                m.setScale(scale, scale);  //设置矩阵缩放比例
        ...
        Bitmap finalBitmap = Bitmap.createBitmap(subsampledBitmap, 0, 0, subsampledBitmap.getWidth(), subsampledBitmap
                .getHeight(), m, true);  //根据矩阵创建了一个新的bitmap
