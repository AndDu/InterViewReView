...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
    ...
    @Override
    public void run() {
        if (waitIfPaused()) return;//任务是否该终止,为什么终止？什么时候终止?
        ...
        Bitmap bmp;
        try {
            checkTaskNotActual();//这个方法很悬，检查任务是否实际的任务  这个方法会抛出TaskCancelledException,当捕获这个任务时，会取消任务。

            bmp = configuration.memoryCache.get(memoryCacheKey);//这里有一段程序执行没看到
            if (bmp == null || bmp.isRecycled()) {
                ...
                if (options.shouldPreProcess()) {
                    ...
                    bmp = options.getPreProcessor().process(bmp);//回调预处理那个方法。
                    if (bmp == null) {
                ...
                if (bmp != null && options.isCacheInMemory()) {
                    ...
                    configuration.memoryCache.put(memoryCacheKey, bmp);//保存进内存
                }
            } else {
            ...
            if (bmp != null && options.shouldPostProcess()) {
                ...
                bmp = options.getPostProcessor().process(bmp);//回调延迟处理那个方法
                if (bmp == null) {
     ...
     *                                this moment)根据uri和imageview的hashcode是否匹配来确定当前任务是否是实际该执行的
     */
    ...
    private boolean isViewCollected() {
        if (imageAware.isCollected()) {//iscollected判断view是否已经被回收
     ...
     */
    private boolean isViewReused() { //通过该View的cachekey判断是否该view是否是被复用来显示另一张图片。
        String currentCacheKey = engine.getLoadingUriForView(imageAware);//通过view的hashcode 来取出缓存的key
        // Check whether memory cache key (image URI) for current ImageAware is actual.
        // If ImageAware is reused for another task then current task should be cancelled.如果两个key不等，当前任务应该取消。
