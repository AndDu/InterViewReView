...
public class ImageLoader {
    ...
    public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
        ...
        if (TextUtils.isEmpty(uri)) { //url为null “” 时直接加载本地图片
        ...
        if (targetSize == null) {
            targetSize = ImageSizeUtils.defineTargetSizeForView(imageAware, configuration.getMaxImageSize());//获取设置好的图片的尺寸
        }
        String memoryCacheKey = MemoryCacheUtils.generateKey(uri, targetSize);//根据图片尺寸和路径生成相应的key
        engine.prepareDisplayTaskFor(imageAware, memoryCacheKey);//将imageview的hashcode和url和尺寸拼接起来的字符串把保存在一个hashmap中，然后...

        listener.onLoadingStarted(uri, imageAware.getWrappedView());//

        Bitmap bmp = configuration.memoryCache.get(memoryCacheKey);// //通过对应的key(url和尺寸组成)取出
        if (bmp != null && !bmp.isRecycled()) {
            ...
            } else if (options.isResetViewBeforeLoading()) {//在加载图片前是否至空imageview
                imageAware.setImageDrawable(null);
            }
